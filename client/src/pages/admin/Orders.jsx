  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="flex gap-1 p-1 rounded-xl" style={{ background:"#1A1208", border:"1px solid #3D2E22" }}>
          {[{val:"",label:"All"},{val:"pending",label:"Pending"},{val:"processing",label:"Process"},{val:"completed",label:"Done"}].map(f => (
            <button key={f.val} onClick={() => { setStatusFilter(f.val); setPage(1); }}
              ...
              style={statusFilter===f.val ? {background:"linear-gradient(135deg,#E8A045,#C8832A)",color:"#1C1410"} : {color:"#8A7060"}}
              onMouseEnter={e => { if (statusFilter!==f.val) { e.currentTarget.style.background="rgba(232,160,69,0.08)"; e.currentTarget.style.color="#F0E6D8"; }}}
              onMouseLeave={e => { if (statusFilter!==f.val) { e.currentTarget.style.background="transparent"; e.currentTarget.style.color="#8A7060"; }}}
            >{f.label}</button>
          ))}
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {orders.length === 0 ? (
                  ...
                  <tr key={o.id} className="transition-colors duration-200" style={{borderBottom:"1px solid #3D2E22"}}
                    onMouseEnter={e => e.currentTarget.style.background="rgba(232,160,69,0.02)"}
                    onMouseLeave={e => e.currentTarget.style.background="transparent"}>
                    <td className="px-5 py-3.5 font-mono text-[11px]" style={{color:"#8A7060"}}>#{o.id}</td>
                    ...
                    <td className="px-5 py-3.5">
                      <span className="text-[10px] font-semibold px-2 py-1 rounded-full"
                        style={o.order_type==="delivery" ? {background:"rgba(139,92,246,0.15)",color:"#A78BFA",border:"1px solid rgba(139,92,246,0.2)"} : {background:"rgba(232,160,69,0.1)",color:"#E8A045",border:"1px solid rgba(232,160,69,0.2)"}}>
                        {o.order_type==="delivery" ? "🛵 Delivery" : "🏪 Pickup"}
                      </span>
                    </td>
                    <td className="px-5 py-3.5"><StatusBadge s={o.status} /></td>
                    <td className="px-5 py-3.5 text-[11px]" style={{color:"#8A7060"}}>
          ...
          <div className="flex gap-2">
            {[{label:"← Prev",disabled:page===1,onClick:()=>setPage(p=>Math.max(1,p-1))},{label:"Next →",disabled:page*LIMIT>=total,onClick:()=>setPage(p=>p+1)}].map(b => (
              <button key={b.label} onClick={b.onClick} disabled={b.disabled}
                ...
                style={{background:"#251C16",border:"1px solid #3D2E22",color:"#B09880"}}
                onMouseEnter={e=>{ if(!b.disabled) e.currentTarget.style.background="#2E2218"; }}
                onMouseLeave={e=>e.currentTarget.style.background="#251C16"}
              >{b.label}</button>
            ))}
          </div>
        </div>
      ...
      {detail && (
        <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in" style={{background:"rgba(0,0,0,0.7)"}} onClick={() => setDetail(null)}>
          <div className="rounded-2xl w-[500px] max-h-[85vh] overflow-y-auto animate-slide-up" style={{background:"#251C16",border:"1px solid #3D2E22",boxShadow:"0 24px 64px rgba(0,0,0,0.6)"}} onClick={e=>e.stopPropagation()}>
            <div className="p-6 flex justify-between items-start" style={{borderBottom:"1px solid #3D2E22"}}>
              <div>
              ...
              <button onClick={() => setDetail(null)} className="text-codex-muted hover:text-codex-text text-xl w-8 h-8 flex items-center justify-center rounded-lg transition-colors" style={{background:"#1C1410"}}>✕</button>
            </div>
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div className="rounded-xl p-3.5" style={{background:"#1C1410",border:"1px solid #3D2E22"}}><p className="text-[10px] uppercase tracking-wider mb-1 text-codex-muted">Status</p><StatusBadge s={detail.status} /></div>
                <div className="rounded-xl p-3.5" style={{background:"#1C1410",border:"1px solid #3D2E22"}}><p className="text-[10px] uppercase tracking-wider mb-1 text-codex-muted">Type</p><p className="font-semibold text-codex-text capitalize">{detail.order_type==="delivery"?"🛵 Delivery":"🏪 Pickup"}</p></div>
                {detail.promo_code && <div className="rounded-xl p-3.5 col-span-2" style={{background:"#1C1410",border:"1px solid #3D2E22"}}><p className="text-[10px] uppercase tracking-wider mb-1 text-codex-muted">Promo</p><p className="font-semibold text-codex-accent">{detail.promo_code}</p></div>}
              </div>
              <div className="rounded-xl overflow-hidden" style={{border:"1px solid #3D2E22"}}>
                <table className="w-full text-sm">
                  <thead style={{background:"#1A1208"}}><tr>
                  ...
                  <tbody>
                    {(detail.items||[]).map((item,i) => (
                      <tr key={i} style={{borderTop:"1px solid #3D2E22"}}>
              ...
              <div className="text-sm space-y-1.5 rounded-xl p-4" style={{background:"#1C1410",border:"1px solid #3D2E22"}}>
                ...
                <div className="flex justify-between font-bold text-base pt-2.5 mt-2" style={{borderTop:"1px solid #3D2E22"}}><span className="text-codex-text">Total</span><span className="text-codex-accent">{fmt(detail.total_amount)}</span></div>
              </div>
              <div className="flex gap-2">
