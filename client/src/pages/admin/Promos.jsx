  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        {promos.map(p => (
          <div key={p.id} className={`glass-card rounded-2xl p-5 transition-all duration-500 group relative overflow-hidden ${!p.is_active ? "opacity-50" : ""}`}>
            ...
            <div className="flex justify-between items-start mb-3 relative z-10">
              <div>
                <span className="font-display text-xl font-bold text-codex-dark tracking-wider">{p.code}</span>
                <div className="flex gap-2 mt-1.5">
                  ...
                  {p.max_discount && <span className="text-[10px] text-codex-muted bg-gray-50 px-2 py-1 rounded-full">max {fmt(p.max_discount)}</span>}
                </div>
              </div>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
          <div className="bg-white rounded-3xl w-[420px] shadow-2xl animate-slide-up">
            <div className="p-7 border-b border-gray-100">
              <h2 className="font-display text-xl font-bold text-codex-dark">{modal.mode === "create" ? "Add Promo" : "Edit Promo"}</h2>
            </div>
            <div className="p-7">
              {error && <p className="text-red-500 text-sm mb-4 bg-red-50 px-4 py-2.5 rounded-xl ring-1 ring-red-100">{error}</p>}
              <form onSubmit={handleSave} className="space-y-4">
                <div><label className="block text-xs font-semibold text-codex-muted uppercase tracking-wider mb-1.5">Code *</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-xs font-semibold text-codex-muted uppercase tracking-wider mb-1.5">Type *</label>
                    <select value={form.type} onChange={e => setForm({...form, type: e.target.value})} className="input-field">
                      <option value="percent">Percentage (%)</option>
                      <option value="fixed">Fixed (Rp)</option>
                    </select></div>
                  <div><label className="block text-xs font-semibold text-codex-muted uppercase tracking-wider mb-1.5">Value *</label>
                  ...
                  <div><label className="block text-xs font-semibold text-codex-muted uppercase tracking-wider mb-1.5">Max Discount</label>
                  ...
                  <div><label className="block text-xs font-semibold text-codex-muted uppercase tracking-wider mb-1.5">Min Order</label>
