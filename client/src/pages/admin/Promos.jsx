...
export default function AdminPromos() {
  ...
  const toggleActive = async (p) => {
    try {
      await updatePromo(p.id, { is_active: !p.is_active });
      load();
    } catch (e) {
      alert(e.message);
    }
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        {promos.map((p) => (
          <div
            key={p.id}
            className={`rounded-2xl p-5 relative overflow-hidden group transition-all duration-500 ${!p.is_active ? "opacity-50" : ""}`}
            style={card}
            onMouseEnter={(e) => {
              e.currentTarget.style.border = "1px solid rgba(156,107,63,0.2)";
              e.currentTarget.style.boxShadow = "0 8px 32px rgba(42,27,14,0.5)";
            }}
            onMouseLeave={(e) => {
          ...
          >
            ...
            <div className="flex justify-between items-start mb-3 relative z-10">
              <div>
                <span className="text-xl font-bold text-codex-text tracking-wider">
                  {p.code}
                </span>
                <div className="flex gap-2 mt-1.5">
                  ...
                  {p.max_discount && (
                    <span
                      className="text-[10px] px-2 py-1 rounded-full text-codex-muted"
                      style={{
                        background: "#F4ECDF",
                        border: "1px solid #E8DCC4",
                      }}
                    >
                      max {fmt(p.max_discount)}
                    </span>
                  )}
                </div>
              </div>
            ...
            <p className="text-codex-muted text-sm relative z-10">
              {p.description}
            </p>
            {Number(p.min_order) > 0 && (
              <p className="text-xs text-codex-muted/60 mt-1 relative z-10">
                Min. order: {fmt(p.min_order)}
              </p>
            )}
            <p className="text-xs text-codex-muted/60 mt-1 relative z-10">
              Expires:{" "}
      ...
      <div className="flex justify-center gap-2">
        <button
          ...
          onMouseEnter={(e) => (e.currentTarget.style.background = "#FFFBF3")}
      ...
      {modal && (
        ...
        >
          <div
            className="rounded-2xl w-[420px] animate-slide-up"
            style={{
              background: "#FFFFFF",
              border: "1px solid #E8DCC4",
              boxShadow:
          ...
          >
            <div className="p-6" style={{ borderBottom: "1px solid #E8DCC4" }}>
              <h2 className="text-xl font-bold text-codex-text">
                {modal.mode === "create" ? "Add Promo" : "Edit Promo"}
              </h2>
            </div>
            <div className="p-6">
              {error && (
