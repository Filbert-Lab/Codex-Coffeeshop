...
const STATUS_TEXT = {
  requesting_media: "Accessing microphone…",
...
export default function CallModal({ webrtc, role, onClose }) {
  ...
  // Reset unread badge whenever the chat panel is open (and on new messages
  // while it stays open). Closing the panel freezes seenCount so only
  // messages arriving afterward count as unread.
  useEffect(() => {
    if (showChat) setSeenCount(chatMessages.length);
      ...
      >
            ...
            >
              <span
                ...
                style={{
                  ...
                  boxShadow: isActive ? "0 0 8px #5A9070" : "0 0 8px #C8924E",
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 flex items-center justify-center gap-3 z-20">
          ...
          <div
            ...
            style={{ background: "rgba(184,139,90,0.2)" }}
          />
          ...
          <button
            ...
            style={{
              background:
                "linear-gradient(135deg, #B85450 0%, #963A37 50%, #7A2A27 100%)",
              color: "#FAF6EF",
              boxShadow:
                "0 4px 20px rgba(184,84,80,0.4), 0 1px 0 rgba(255,255,255,0.15) inset",
            }}
            aria-label="End call"
            title="End call"
          >
        ...
        {status === "ended" && (
          <div
            className="absolute inset-0 flex flex-col items-center justify-center gap-4 z-30 animate-fade-in"
